...
contract TestStaking is Initializable, OwnableUpgradeSafe {
    ...
    function initialize(IERC20 _contractAddress) initializer public {
    ...
    // function stake(uint _stake, address referree) external {
    ...
    //     require(referree != address(0));
    //     //require(IERC20(token).transfer(address(this), _stake), "Stake failed due to failed amount transfer.");
    ...
    //         // staker should approve tokens for transfer
    //         contractAddress.approve(address(this), _stake);
    ...
    //     require(stakeholders[_stakeholder].staker == false);
